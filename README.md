...
- **Admin panel** (`/admin`) — dodavanje/izmena proizvoda i slika, hero slike, pregled i status porudžbina, podešavanja (cena dostave, podaci radnje, lozinka).
...
Bitno:
- Folder `server/data/` (baza) i `public/uploads/` (slike) moraju biti na **trajnom disku** koji se ne briše pri redeploy-u — kod Railway/Render ovo se zove "persistent volume/disk", obavezno uključi.
...
- Email/telefon radnje — prikazuje se u footeru sajta.

## Slike proizvoda

Kad budeš spreman da ubaciš slike sa WeTransfer-a, samo uđi u Admin → Proizvodi → otvori proizvod → "+ Dodaj slike", i izaberi fajlove sa računara (one koje preuzmeš sa WeTransfer linka). Isto za hero slike.
